...
def test_constraint_violation():
    ...
    assert are_constraints_violated(block, {'color': ['red', 'red', 'blue', 'blue']}, enumer, 4, 0) == False
    assert are_constraints_violated(block, {'color': ['red', 'blue', 'red', 'blue']}, enumer, 4, 0) == True

    block = fully_cross_block([color, text, con_factor_within_trial],
                              [color, text],
                              [AtMostKInARow(2, (color, red_color))])

    assert are_constraints_violated(block, {'color': ['red', 'blue', 'blue', 'blue']}, enumer, 4, 0) == False
    assert are_constraints_violated(block, {'color': ['red', 'red', 'blue', 'blue']}, enumer, 4, 0) == False
    assert are_constraints_violated(block, {'color': ['red', 'red', 'red', 'blue']}, enumer, 4, 0) == True
    assert are_constraints_violated(block, {'color': ['blue', 'red', 'red', 'red']}, enumer, 4, 0) == True
