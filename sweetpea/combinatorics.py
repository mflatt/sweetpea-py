...
def count_remaining_permutations(counters: List[int]) -> int:
    return factorial(sum(counters)) // reduce(lambda d, c: d * factorial(c), counters, 1)
...
def count_permutation_with_copies(q: int, m: int, first_n: int) -> int:
    ...
    if n == first_n:
        ...
        return count_prefixes_of_permutations_with_copies(q, m, first_n)
...
def count_prefixes_of_permutations_with_copies(q: int, m: int, first_n: int,
    ...
    if first_n <= m:
        ...
        return k_prefixes_of_permutations_with_copies(q, m, first_n, -1, pmemo)
...
def compute_jth_prefix_of_permutations_with_copies(q: int, m: int, first_n: int, j: int,
    ...
    if first_n <= m:
        ...
        return k_prefixes_of_permutations_with_copies(q, m, first_n, j, pmemo)
    ...
    while ks:
        ...
        if k == DoCount:
            ...
            if need_n == 0:
                ...
                value = memo.get((start_i, need_n), None)
                if value and find > -1:
                    total = value * multiplier
                    if find < total:
                        value = None # explore this subtree, so we find the permutation
                    else:
                        find -= total
                if not value:
...
def buckets_to_counters(buckets: Any, q: int) -> List[int]:
    ...
    rev_buckets = ()
    while buckets != ():
        rev_buckets = (buckets[0], rev_buckets)
        buckets = buckets[1]
    i = 0;
    while rev_buckets != ():
